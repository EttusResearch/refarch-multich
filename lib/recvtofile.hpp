...
void recvToFileMultithread16(uhd::rx_streamer::sptr rx_stream,
        const std::string& cpu_format,
        const std::string& wire_format,
        const std::string& file,
        size_t samps_per_buff,
    ...
    for (size_t i = 0; i < buffs.size(); i++) {
        ...
        const std::string this_filename = ReceiveFunctions::generateOutFilenameMultithread16(file, buffs.size(), rx_identifier * 2 + i, tx_chan_num, numrepeat, signalSettings, deviceSettings, threadnum);
