...
void recvToFile(uhd::rx_streamer::sptr rx_stream,
        ...
        double timeout, int rx_rate, int tx_chan_num, SignalSettings& signalSettings, int numrepeat, DeviceSettings& deviceSettings, GraphSettings& graphSettings, double time_requested){ 
    ...
    for (size_t i = 0; i < buffs.size(); i++) {
        const std::string this_filename = ReceiveFunctions::generateOutFilename(file, i, tx_chan_num, numrepeat, deviceSettings.tx_freq, deviceSettings.folder_name);
...
void recvToMemMultithread(uhd::rx_streamer::sptr rx_stream,
    ...
    std::cout << "Thread: " << threadnum << " Received: " << num_total_samps << " samples..." << std::endl;
...
void recvToFileMultithread(uhd::rx_streamer::sptr rx_stream,
    ...
    for (size_t i = 0; i < buffs.size(); i++) {
        ...
        const std::string this_filename = ReceiveFunctions::generateOutFilenameMultithread(file, rx_identifier * 2 + i, tx_chan_num, numrepeat, deviceSettings.tx_freq, deviceSettings.folder_name, threadnum);
