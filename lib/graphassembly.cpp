...
void GraphAssembly::buildRadios(GraphSettings& graphSettings)
{
    ...
    for (auto& elem : graphSettings.radio_block_list) {
        ...
        graphSettings.radio_ctrls.push_back(
            graphSettings.graph->get_block<uhd::rfnoc::radio_control>(elem));
        ...
        for (auto& src_block : ddc_sources) {
            if (edge.src_blockid == src_block && edge.src_port == ddc_source_port) {
                ...
                if (blockid.match("DDC")) {
                    ...
                    graphSettings.ddc_ctrls.push_back(
                        graphSettings.graph->get_block<uhd::rfnoc::ddc_block_control>(
                            blockid));
                }
            }
        }
        ...
        for (auto& dst_block : duc_destinations) {
            if (edge.dst_blockid == dst_block && edge.dst_port == duc_dst_port) {
                ...
                if (blockid.match("DUC")) {
                    ...
                    graphSettings.duc_ctrls.push_back(
                        graphSettings.graph->get_block<uhd::rfnoc::duc_block_control>(
                            blockid));
                }
            }
        }
...
void GraphAssembly::buildReplay(GraphSettings& graphSettings)
{
    ...
    for (auto& replay_id : graphSettings.replay_block_list) {
        ...
        graphSettings.replay_ctrls.push_back(
            graphSettings.graph->get_block<uhd::rfnoc::replay_block_control>(replay_id));
        graphSettings.replay_ctrls.push_back(
            graphSettings.graph->get_block<uhd::rfnoc::replay_block_control>(replay_id));
    }
    ...
    for (auto& rctrl : graphSettings.replay_ctrls) {
        std::cout << "Using Replay " << rctrl->get_block_id() << ", Channel "
                  << graphSettings.replay_chan_vector[count_replay_chan] << std::endl;
        count_replay_chan++;
    }
}

void GraphAssembly::buildStreams(GraphSettings& graphSettings,
    ...
    const std::string& otw)
{
    ...
    graphSettings.rx_stream = graphSettings.graph->create_rx_streamer(
        graphSettings.radio_ctrls.size(), stream_args);
    ...
    for (int i_s2r = 0; i_s2r < graphSettings.replay_ctrls.size(); i_s2r = i_s2r + 2) {
        streamer_args["block_id"] =
            graphSettings.replay_ctrls[i_s2r]->get_block_id().to_string();
        streamer_args["block_port"] = std::to_string(0);
        stream_args.args            = streamer_args;
        stream_args.channels        = {0};

        graphSettings.tx_stream = graphSettings.graph->create_tx_streamer(
...
void GraphAssembly::connectGraph(GraphSettings& graphSettings)
{
    ...
    for (int i = 0; i < graphSettings.radio_ctrls.size(); i++) {
        ...
        graphSettings.graph->connect(graphSettings.radio_block_list[i],
            0,
            graphSettings.ddc_ctrls[i]->get_block_id(),
            0);
        std::cout << "Connected " << graphSettings.radio_block_list[i] << " to "
                  << graphSettings.ddc_ctrls[i]->get_block_id() << std::endl;

        graphSettings.graph->connect(
            graphSettings.ddc_ctrls[i]->get_block_id(), 0, graphSettings.rx_stream, i);
        std::cout << "Connected " << graphSettings.ddc_ctrls[i]->get_block_id() << " to "
                  << graphSettings.rx_stream << " Port " << i << std::endl;
    }

    int pos2 = 0;
    if (graphSettings.duc_ctrls.size() > 0) {
        for (auto& rctrl : graphSettings.radio_ctrls) {
            graphSettings.graph->connect(graphSettings.duc_ctrls[pos2]->get_block_id(),
                graphSettings.duc_chan,
                rctrl->get_block_id(),
                0);
            std::cout << "Connected " << graphSettings.duc_ctrls[pos2]->get_block_id()
                      << " port " << graphSettings.duc_chan << " to radio "
        ...
        for (int i_r2d = 0; i_r2d < graphSettings.duc_ctrls.size(); i_r2d++) {
            ...
            while (true) {
                try {
                    graphSettings.graph->connect(
                        graphSettings.replay_ctrls[i_r2d]->get_block_id(),
                        graphSettings.replay_chan_vector[i_r2d],
                        graphSettings.duc_ctrls[i_r2d]->get_block_id(),
                        graphSettings.duc_chan);
                    break;
                } catch (...) {
            ...
            std::cout << "Connected " << graphSettings.replay_ctrls[i_r2d]->get_block_id()
                      << " port " << graphSettings.replay_chan_vector[i_r2d] << " to DUC "
                      << graphSettings.duc_ctrls[i_r2d]->get_block_id() << " port "
                      << graphSettings.duc_chan << std::endl;
        }
        std::this_thread::sleep_for(std::chrono::milliseconds(1000));
    ...
    for (int i_s2r = 0; i_s2r < graphSettings.replay_ctrls.size(); i_s2r = i_s2r + 2) {
        graphSettings.graph->connect(graphSettings.tx_stream_vector[i_s2r],
            0,
            graphSettings.replay_ctrls[i_s2r]->get_block_id(),
            0);
        std::cout << "Streamer: " << graphSettings.tx_stream_vector[i_s2r]
                  << " connected to " << graphSettings.replay_ctrls[i_s2r]->get_block_id()
                  << std::endl;
    }
}
...
void GraphAssembly::connectGraphMultithread(GraphSettings& graphSettings)
{
    ...
    for (int j = 0; j < graphSettings.ddc_ctrls.size(); j++) {
        ...
        graphSettings.graph->connect(graphSettings.ddc_ctrls[j]->get_block_id(),
            0,
            graphSettings.rx_stream_vector[j],
            graphSettings.replay_chan_vector[j]);
        std::cout << "Connected " << graphSettings.ddc_ctrls[j]->get_block_id() << " to "
                  << graphSettings.rx_stream_vector[j] << " Port "
                  << graphSettings.replay_chan_vector[j] << std::endl;
    }
...
void GraphAssembly::buildStreamsMultithread(GraphSettings& graphSettings,
    const std::string& streamargs,
    const std::string& format,
    const std::string& otw)
{
    ...
    uhd::stream_args_t stream_args(
        format, otw); // We should read the wire format from the blocks
    stream_args.args = streamer_args;
    std::cout << "Using streamer args: " << stream_args.args.to_string() << std::endl;
