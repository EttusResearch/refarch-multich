...
int BlockSettings::setRadioRates(GraphSettings& graphSettings, double& rx_rate, double& tx_rate){
     ...
     if (graphSettings.ddc_ctrls.size() > 0) {
        ...
        for (auto& rctrl : graphSettings.radio_ctrls){
            rx_rate = rctrl->set_rate(rx_rate);
             std::cout << boost::format("Actual RX Rate: %f Msps...") % (rx_rate / 1e6) << std::endl
              << std::endl;
        }
    }
...
void BlockSettings::tuneRX(GraphSettings& graphSettings, const double& rx_freq){
    ...
    for (auto& rctrl : graphSettings.radio_ctrls){
       ...
       //Set USRP RX Frequency for All Devices
        std::cout << std::fixed; 
        std::cout << rctrl->get_block_id() << " Setting RX Freq: " << std::fixed << (rx_freq / 1e6) << " MHz..."
                << std::endl;
...
void BlockSettings::tuneTX(GraphSettings& graphSettings, const double& tx_freq){
   ...
   for (auto& rctrl : graphSettings.radio_ctrls){
    ...
    std::cout << std::fixed;
    std::cout << rctrl->get_block_id() << " Setting TX Freq: " << std::fixed << (tx_freq / 1e6) << " MHz..."
            << std::endl;
...
void BlockSettings::setRXGain(GraphSettings& graphSettings, const double& rx_gain){
    ...
    for (auto& rctrl : graphSettings.radio_ctrls){
        ...
        std::cout << std::fixed;
        std::cout << rctrl->get_block_id() << " Setting RX Gain: " << rx_gain << " dB..." << std::endl;
...
void BlockSettings::setTXGain(GraphSettings& graphSettings, const double& tx_gain){
    ...
    for (auto& rctrl : graphSettings.radio_ctrls){
        ...
        std::cout << std::fixed;
        std::cout << rctrl->get_block_id() << " Setting TX Gain: " << tx_gain << " dB..." << std::endl;
...
void BlockSettings::setRXBw(GraphSettings& graphSettings, const double& rx_bw){
    ...
    if(rx_bw > 0){
        for (auto& rctrl : graphSettings.radio_ctrls){
            std::cout << std::fixed;
            std::cout << rctrl->get_block_id() << " Setting RX Bandwidth: " << (rx_bw / 1e6) << " MHz..." << std::endl;
...
void BlockSettings::setTXBw(GraphSettings& graphSettings, const double& tx_bw){
    ...
    if(tx_bw > 0){
        for (auto& rctrl : graphSettings.radio_ctrls){
            ...
            std::cout << std::fixed;
            std::cout << rctrl->get_block_id() << " Setting TX Bandwidth: " << (tx_bw / 1e6) << " MHz..." << std::endl;
