...
void GraphAssembly::connectGraph(GraphSettings& graphSettings, SignalSettings& signalSettings){
    ...
    //TODO: Add graph for single rx streamer when using single loopback mode
    ...
    if (signalSettings.singleTXRX_loopback == false){
        ...
        std::this_thread::sleep_for(std::chrono::milliseconds(1000));
/* 
        std::cout << "Connecting Second Radio..." << std::endl;
        graphSettings.graph->connect(graphSettings.radio_block_list[signalSettings.singleRX + 1], 0, graphSettings.ddc_ctrls[signalSettings.singleRX + 1]->get_block_id(),0);
        std::cout << "Connected "  << graphSettings.radio_block_list[signalSettings.singleRX + 1] << " to " << graphSettings.ddc_ctrls[signalSettings.singleRX + 1]->get_block_id() << std::endl;
        graphSettings.graph->connect(graphSettings.ddc_ctrls[signalSettings.singleRX + 1]->get_block_id(), 0, graphSettings.rx_stream,1);
        std::cout << "Connected " << graphSettings.ddc_ctrls[signalSettings.singleRX + 1]->get_block_id() << " to " << graphSettings.rx_stream << " Port " << 1 << std::endl;
        std::this_thread::sleep_for(std::chrono::milliseconds(1000)); */
...
void GraphAssembly::buildStreamsMultithread(GraphSettings& graphSettings, DeviceSettings& deviceSettings, SignalSettings& signalSettings){
    ...
    //Changing to one rx stream per radio, for multithreading. 
