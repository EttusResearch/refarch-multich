...
void BlockSettings::tuneRX(GraphSettings& graphSettings, DeviceSettings& deviceSettings, ProgramMetaData& pmd){
    ...
    for (auto& rctrl : graphSettings.radio_ctrls){
        ...
        //Need to look at this method of tune request
        /* if (vm.count("rx_args")) {
            rctrl->set_rx_tune_args(rx_args, 0);
        } */
/* 
        uhd::tune_request_t tune_request(deviceSettings.rx_freq);
        if (pmd.vm.count("int-n")) {
            tune_request.args = uhd::device_addr_t("mode_n=integer");
        } */
        
       //Set USRP RX for All Devices
        std::cout << std::fixed; 
        std::cout << rctrl->get_block_id() << " Setting RX Freq: " << std::fixed << (deviceSettings.rx_freq / 1e6) << " MHz..."
                << std::endl;

        rctrl->set_rx_frequency(deviceSettings.tx_freq, 0);
...
void BlockSettings::tuneTX(GraphSettings& graphSettings, DeviceSettings& deviceSettings, ProgramMetaData& pmd){
   ...
   /*   if (vm.count("tx_args")) {
            rctrl->set_tx_tune_args(tx_args, 0);
        }
    */
        //Set USRP TX for All devices
        std::cout << std::fixed;
