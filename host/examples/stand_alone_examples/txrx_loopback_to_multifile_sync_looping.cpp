    ...
    uhd::usrp::multi_usrp::sptr usrp)
{
    ...
    while (not stop_signal_called && loop_counter < tx_burst_reps) {
...
int UHD_SAFE_MAIN(int argc, char* argv[])
{
    ...
    std::string rx_args, file, type, rx_ant, rx_subdev, rx_channels, enable_lo;
    size_t total_num_samps, spb;
    double rx_rate, rx_freq, rx_gain, rx_bw;
    double settling, rx_timeout, tx_burst_reps;
    int runs;
    ...
    desc.add_options()
        ...
        ("enable_lo", po::value<std::string>(&enable_lo)->default_value("none"), "enable_lo options (none, n320, n321)")
        ("tx_burst_reps", po::value<double>(&tx_burst_reps)->default_value(double(1)), "number of wavetable buffers to transmit on tx")
    ...
    std::cout << std::endl;
    std::cout << boost::format("Creating the transmit/receive usrp device with: %s...") % tx_args
              << std::endl;
    ...
    if (vm.count("rx-subdev"))
        tx_usrp->set_rx_subdev_spec(rx_subdev);
    ...
    for (size_t ch = 0; ch < rx_channel_strings.size(); ch++) {
        ...
        if (chan >= tx_usrp->get_rx_num_channels()) {
    ...
    if (vm.count("ref")) {
        ...
        std::cout << boost::format("Setting device clock source to %s and time source to %s") % tx_usrp->get_clock_source(0) % tx_usrp->get_time_source(0) << std::endl;
    }
    ...
    uhd::time_spec_t temp_set_command_time = tx_usrp->get_time_now() + uhd::time_spec_t(settling);
    tx_usrp->set_command_time(temp_set_command_time); 
    ...
    */
    ...
    std::cout << boost::format("Setting RX Rate: %f Msps...") % (rx_rate / 1e6)
              << std::endl;
    tx_usrp->set_rx_rate(rx_rate); //rx_usrp
    std::cout << boost::format("Actual RX Rate: %f Msps...")
                     % (tx_usrp->get_rx_rate() / 1e6) //rx_usrp
              << std::endl
              << std::endl;
    ...
    if(enable_lo == "n321") 
    {
        ...
        tx_usrp->set_tx_lo_export_enabled(true, "lo1", 0);
        tx_usrp->get_device()->get_tree()->access<bool>("mboards/0/dboards/A/tx_frontends/0/los/lo1/lo_distribution/LO_OUT_1/export").set(true);
        tx_usrp->get_device()->get_tree()->access<bool>("mboards/0/dboards/A/tx_frontends/0/los/lo1/lo_distribution/LO_OUT_0/export").set(true);
        tx_usrp->get_device()->get_tree()->access<bool>("mboards/0/dboards/A/tx_frontends/0/los/lo1/lo_distribution/LO_OUT_2/export").set(true);
        tx_usrp->get_device()->get_tree()->access<bool>("mboards/0/dboards/A/tx_frontends/0/los/lo1/lo_distribution/LO_OUT_3/export").set(true);
    ...
    for(auto channel : rx_channel_nums)
    {
        // Route the LO to the respective channel for both the N321 and N320
        if(enable_lo == "n321" || enable_lo == "n320") 
        {
            tx_usrp->set_rx_lo_source("external", "lo1", channel); //rx_usrp
            std::cout << boost::format("Setting RX LO source for channel %1%...") % channel << std::endl;
        }
    }

    for(auto channel : tx_channel_nums)
    {
        // Route the LO to the respective channel for both the N321 and N320
        if(enable_lo == "n321" || enable_lo == "n320") 
        {
            tx_usrp->set_tx_lo_source("external", "lo1", channel);
    ...
    for (size_t ch = 0; ch < rx_channel_nums.size(); ch++) {
        ...
        tx_usrp->set_rx_freq(rx_tune_request, channel); //rx_usrp
        std::cout << boost::format("Actual RX Freq: %f MHz...")
                         % (tx_usrp->get_rx_freq(channel) / 1e6) //rx_usrp
                  << std::endl
                  << std::endl;
        ...
        if (vm.count("rx-gain")) {
            std::cout << boost::format("Setting RX Gain: %f dB...") % rx_gain
                      << std::endl;
            tx_usrp->set_rx_gain(rx_gain, channel); //rx_usrp
            std::cout << boost::format("Actual RX Gain: %f dB...")
                             % tx_usrp->get_rx_gain(channel) //rx_usrp
                      << std::endl
                      << std::endl;
        }
        ...
        if (vm.count("rx-bw")) {
            std::cout << boost::format("Setting RX Bandwidth: %f MHz...") % (rx_bw / 1e6)
                      << std::endl;
            tx_usrp->set_rx_bandwidth(rx_bw, channel); //rx_usrp
            std::cout << boost::format("Actual RX Bandwidth: %f MHz...")
                             % (tx_usrp->get_rx_bandwidth(channel) / 1e6) //rx_usrp
                      << std::endl
                      << std::endl;
        }
        ...
        if (vm.count("rx-ant"))
            tx_usrp->set_rx_antenna(rx_ant, channel); //rx_usrp
    }
    ...
    rx_sensor_names = tx_usrp->get_rx_sensor_names(0); //rx_usrp
    if (std::find(rx_sensor_names.begin(), rx_sensor_names.end(), "lo_locked")
        != rx_sensor_names.end()) {
        uhd::sensor_value_t lo_locked = tx_usrp->get_rx_sensor("lo_locked", 0); //rx_usrp
...
for (int i = 0; i < runs; i++)
{
    ...
    uhd::time_spec_t tx_rx_shared_start_time = uhd::time_spec_t(tx_usrp->get_time_now() + uhd::time_spec_t(settling));
    ...
    std::cout << boost::format("get_time_now in main...%s.%s") % tx_usrp->get_time_now().get_full_secs() % tx_usrp->get_time_now().get_frac_secs() << std::endl;
    ...
    transmit_thread.create_thread(boost::bind(
        &transmit_worker, buff, wave_table, tx_stream, md, step, index, num_channels, tx_burst_reps, tx_usrp));

    // recv to file
    if (type == "double")
        recv_to_file<std::complex<double>>(
            tx_usrp, "fc64", otw, file_run, spb, total_num_samps, tx_rx_shared_start_time, rx_channel_nums, rx_timeout); //rx_usrp
    else if (type == "float")
        recv_to_file<std::complex<float>>(
            tx_usrp, "fc32", otw, file_run, spb, total_num_samps, tx_rx_shared_start_time, rx_channel_nums, rx_timeout); //rx_usrp
    else if (type == "short")
        recv_to_file<std::complex<short>>(
            tx_usrp, "sc16", otw, file_run, spb, total_num_samps, tx_rx_shared_start_time, rx_channel_nums, rx_timeout); //rx_usrp
    else {
    ...
    //debug
    std::cout << boost::format("after recv loop...%s.%s") % tx_usrp->get_time_now().get_full_secs() % tx_usrp->get_time_now().get_frac_secs() << std::endl;
